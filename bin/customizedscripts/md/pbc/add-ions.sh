...
radius=$3
postfix="iond"
outputname="${filename}_wb_${radius}A_${postfix}"
...
set dir ${dir}
set molename ${filename}_wb_${radius}A
...
if [[ -s ${dir}/${filename}_wb_${radius}A_ion.psf ]];then
    echo "WARNING: Fail to add ions."
    rm ${dir}/${filename}_wb_${radius}A_ion.psf ${dir}/${filename}_wb_${radius}A_ion.pdb
fi

echo -e "ionization done.\n"
else
    echo "Syntax: add-ions.sh parent-dir filename padding"
fi
